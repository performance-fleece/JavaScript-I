...
function findCar(car) {
    for (let i = 0 ; i < inventory.length; i++) {
        ...
        if (currentCar.id === car) {
            return `Car ${car} is a ${currentCar.car_year} ${currentCar.car_make} ${currentCar.car_model}`
        }
    }
}
...
let lastCar = 0;
console.log(findCar(inventory.length));
...
for (let i = 0; i < inventory.length; i++) {
    currentCar = inventory[i].car_model;
    carModels.push(currentCar)
}
...
for (let i = 0; i <inventory.length; i++) {
    currentCar = inventory[i].car_year;
...
for (let i = 0; i < inventory.length; i++) {
    currentCar = inventory[i];
    if (currentCar.car_make == "Audi" || currentCar.car_make == "BMW") {
